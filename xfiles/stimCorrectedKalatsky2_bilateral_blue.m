function SS = stimCorrectedKalatsky2_bilateral_blue(myScreenInfo,Pars)
% stimKalatsky2_bilateral makes color sweeping bars moving periodically in the x/y direction
% for xpos stimulation, makes two vertical bars, that are mirrored
% 2016-02 DS created from stimCorrectedKalatsky2.m
...
for iframe = 1:nFramesPerSweep
    ...
    elseif xpos
        ...
        ind3 = find(PHI > -angleLimit(2)); %24/2/16 DS
        ind4 = find(PHI < -angleLimit(1)); %24/2/16 DS
    end
    ind = intersect_sorted(ind1,ind2);%fast. applicable only to sorted idx
    %ind = intersect(ind1,ind2);%slow
    
    if xpos  %24/2/16 DS
        ind_mirror = intersect_sorted(ind3, ind4);
        ind = union(ind, ind_mirror);
    end
