function SS = stimOptiWaveOutput_NS(myScreenInfo,Pars)
...
pp{5}  = {'shape1',      'Ramp(1), Step(2) or Sine(3)',  3,1,3};
pp{6}  = {'freq1',       'Frequency of pulses (Hz*10)',       10,0,10000};
pp{7}  = {'dcyc1',       'Pulses Duty Cycle (%*10)',          200, 0, 1000};
pp{8}  = {'tstart2',     'Start time (ms)',                   1000,0,60000};
pp{9}  = {'tend2',       'End time (ms)',                     6000,0,60000};
pp{10}  = {'amp2',        'Amplitude (mV)',                   0,0,5000};
pp{11}  = {'shape2',      'Ramp(1), Step(2) or Sine(3)', 1,1,3};
pp{12}  = {'freq2',       'Frequency of pulses (Hz*10)',      0,0,10000};
pp{13}  = {'dcyc2',       'Pulses Duty Cycle (%*10)',         200, 0, 1000};
pp{14}  = {'trigtype',    'Manual(1), HwDigital(2), Immediate(3)', 1, 1, 3};



x = XFile('stimOptiWaveOutput_NS',pp);
