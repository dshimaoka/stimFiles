function SS = stimWave2TrainEJSweep2(myScreenInfo,Pars)
...
pp{6}  = {'ToneFreq1',     'Tone frequency (Hz*10)',   200,0,10000};
pp{7}  = {'SweepUpDown',    'Sweep (0 for up, 1 for down)', 0,0,1};
pp{8} = {'RiseTime1',   'Rise and fall time (ms)',  5  ,0,500};
% %2nd output
% pp{9}  = {'WaveAmp2',   'Amplitude of sinusoid (mV*1000)',    100, 0, 5000};
% pp{10}  = {'WaveStart2', 'Time of stim onset (ms)',        2000,0,6000};
% pp{11}  = {'WaveStop2', 'Time of stim offset (ms)',        4000,0,6000};
% pp{12}  = {'WaveFreq2',  'Frequency (Hz*10)',             4400,0,20000};
% pp{13}  = {'WaveDurOn2',     'Duration of on period for wave (ms)',   20,0,1000};
% pp{14}  = {'ToneFreq2',     'Tone frequency (Hz*10)',   200,0,10000};
% pp{15} = {'RiseTime2',   'Rise and fall time (ms)',  5  ,0,500};
...
WaveDurOn    = WaveStop - WaveStart;       % considering I want the tonesweep to be continuous, there is no need for the WaveDurOn parameter to be separate
% WaveDurOn(1)    = Pars(6);      % ms
ToneFreq     = Pars(6)/10;   % Hz If 0, just envelope wave
SweepDir     = Pars(7);      % 0: tone starts at ToneFreq/2 and goes up to ToneFreq, 1: tone starts at ToneFreq and goes down to ToneFreq/2
RiseTime     = Pars(8)/1000; %s If 0, no truncation

% WaveAmp(2)      = Pars(9)/1000;
% WaveStart(2)    = Pars(10);     % ms
% WaveStop(2)     = Pars(11);     % ms
% WaveFreq(2)     = Pars(12)/10;  % Hz
% WaveDurOn(2)    = Pars(13);     % ms
% ToneFreq(2)     = Pars(14)/10;    % Hz If 0, just envelope wave
% RiseTime(2)     = Pars(15)/1000;  % s If 0, no truncation
%% Make the stimulus
...
%fs = 125e3; %13.10,20
fs = myScreenInfo.WaveInfo.SampleRate %DS 2015-3-25 % 250e3
nt = ceil(dur*fs);
tt = (1:nt)./fs;
...
% for ch = 1:2
    ...
    if ToneFreq  > 0 %2014/12/9
        ...
        %         tp = 2.^tp;
        tp = linspace(-4,0,length(ttWave));
        if SweepDir == 0;
            SS.WaveStim.Waves(ntWavestart:ntWavestop-2) = ...
                WaveAmp *train_output'.*sin(2*pi*(ToneFreq *2.^tp).*ttWave)';
        elseif SweepDir == 1;
            tp=flip(tp);
            SS.WaveStim.Waves(ntWavestart:ntWavestop-2) = ...
                WaveAmp *train_output'.*sin(2*pi*(ToneFreq *2.^tp).*ttWave)';
