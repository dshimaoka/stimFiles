function SS = stimFlashedGrat_blue(myScreenInfo,Pars)
...
pp{6}  = {'dim1',  'Diameter (deg*10)'             100,  1,   5000};
pp{7}  = {'dim2',  'Diameter (deg*10)'             250,  1,   500};
...
elseif shape==1 % a rectangle
    SS.nImages = 2;
    ImageTextures = cell(SS.nImages,1);
    ImageTextures{1} = 0; % blank
    ImageTextures{2} = uint8(max(min((round(255*mm)*(1+cc)),255),0));
...
elseif shape==2 % a circular gaussian aperture
    ...
    WindowImage = exp(-(xx.^2/(2*sigx) + yy.^2/(2*sigy)));        
