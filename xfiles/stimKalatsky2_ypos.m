function SS = stimKalatsky2_ypos(myScreenInfo,Pars)
%  stimKalatsky_ypos makes color bars moving in the x direction
%
% SS = stimKalatsky_ypos(myScreenInfo,Pars) returns an object SS of type ScreenStim
%
% SS = stimKalatsky_ypos(myScreenInfo) uses the default parameters
%
% 2011-05 AP
...
pp{3}  = {'size',     'Bar width (deg)',     1,0,90};
...
shiftperframe = direction*Ly*tfreq/myScreenInfo.FrameRate;

y0 = direction*Ly*temporalphase/360;

for iframe = 1 : SS.nFrames
    yoffset = ceil(mod(y0+iframe*shiftperframe,Ly));
    SS.SourceRects(:,1,iframe) = [1; 1;  len; wid];
    SS.DestRects(:,1,iframe)  = [0; yoffset; len; wid+yoffset];
end
...
myScreenStim2 = ScreenStim.Make(myScreenInfo,'stimKalatsky_ypos'); %#ok<UNRCH>
