function SS = stimSparseNoiseUncorrBLACKvardel(myScreenInfo,Pars)
...
x = XFile('stimSparseNoiseUncorrBLACKvardel', pp);
...
% create sequence
noise = 1 + cumsum(ceil(1/nFr - rand(1,2*SS.nFrames)));
imgsq = noise(1:SS.nFrames);

SS.nImages = max(noise(:));

% create textures
ImageTextures = cell(SS.nImages,1);
for iImage = 1:SS.nImages
    im = floor(ncs * rand(nr, nc, 1)) / (ncs - 1) * 2 - 1;
    im(rand(nr, nc)<sprsns) = -1; % every square will become BLACK with prob=sprsns
