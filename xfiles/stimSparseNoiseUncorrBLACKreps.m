function SS = stimSparseNoiseUncorrBLACKreps(myScreenInfo,Pars)
...
x = XFile('stimSparseNoiseUncorrBLACKreps', pp);
...
% create sequence
noise = 2*double(rem(1+cumsum(ceil(1/nFr - rand(nr, nc,2*SS.nFrames)), 3), 20)==0)-1;
%noise = double(rem(1+cumsum(ceil(1/nFr - rand(nr, nc,2*SS.nFrames)), 3), 20)==0) .* ...
%	(2*double(rem(1+cumsum(ceil(1/2 - rand(nr, nc,2*SS.nFrames)), 3), 20)==0)-1);
...
SS.ImageSequence = repmat((1:round(SS.nFrames/nreps))', 1, nreps);
