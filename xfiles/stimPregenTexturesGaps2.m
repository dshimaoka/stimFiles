function SS = stimPregenTexturesGaps2(myScreenInfo,Pars)
...
pp{6}  = {'nfON',    'number of frames stimulus stays on (deg*10)', 10,1,200};
pp{7}  = {'nfOFF',   'number of frames stimulus stays off (deg*10)', 10,1,200};
pp{8}  = {'c',      'Contrast (%)',                           100,0,100};
pp{9} = {'seed',   'Seed of random number generator',        1,1,100};

x = XFile('stimPregenTexturesGaps2',pp);
...
%     blockGray = SS.nImages * ones(2*nfOFF, size(imgsq,2));
    ...
    blockGray2(randperm(numel(blockGray2), 9*numel(blockGray2)/10)) = NaN;
    
    imgsq = cat(1, imgsq, blockGray, blockGray2); % last image is grey
