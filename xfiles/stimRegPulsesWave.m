function SS = stimRegPulsesWave(myScreenInfo,Pars)
% stimRegPulsesWave	makes a train of 5 wave stimuli at regular interval +
%			gray screen
%
% SS = stimRegPulsesWave(myScreenInfo,Pars) returns object SS of type ScreenStim
% SS = stimRegPulsesWave(myScreenInfo) uses the default parameters
%
...
SS.Type = 'stimRegPulsesWave';
SS.Parameters = Pars;
% rng(seed); % seeds the random number generator 

% generate 5-pulse train
ss = [];
%beg = 0.1+rand(1)*(dur-0.1-4/f)/3; % randomise beg time (arbitrary definition)
beg = 0.2; % beg time 200ms in 
ss = [beg:1/f:beg+4/f];
...
SS = stimRegPulsesWave(myScreenInfo); %#ok<UNRCH>
