function SS = stimCorrectedKalatsky(myScreenInfo,Pars)
...
pp{3}  = {'start',	'Stimulus start (deg)',      -180,0,180};
pp{4}  = {'end',	'Stimulus end (deg)',        -180,0,180};
pp{5}  = {'tf',     'Temporal frequency (Hz *100)',      1,1,400};
...
x = XFile('stimCorrectedKalatsky',pp);
...
T = 1/tfreq;
nFramesPerSweep = round(myScreenInfo.FrameRate * (T-bdur));
...
shiftperframe = direction*L/(nFramesPerSweep-1); %[rad]
...
%     offset_angle(iframe) =  pi/180*(start_point + mod((iframe-1)*shiftperframe,L));%[rad]
...
% bgIdx = (value == 0);
% value = uint8(255*0.5*(1+cc*value));
% value(bgIdx) = uint8(255*mm);
% value = uint8(255*mm*(1+cc*value));
value = uint8(value);  
