function SS = stimRandNoiseTS2(myScreenInfo,Pars)
...
for iImage = 1:SS.nImages
    ImageTextures{iImage} = 2*round(ncs*rand(nr,nc,1)-0.5)/(ncs-1)-1;% MK found a bug in a previous version. see below
    %ImageTextures{iImage} = 2*round((ncs-1)*rand(nr,nc,1))/(ncs-1)-1;
    %previously, wrong way of calculation.
end
disp('correct distribution')
% ImageTextures{iImage} = 2*round((ncs-1)*rand(nr,nc,1))/(ncs-1)-1
% where     nr - No of rows
%           nc - No of columns
%          ncs - No of contrasts (of gray levels)
% This line generates nc x nr white noise images with ncs gray levels (between -1 and 1). 
% The bug is that the min and the max values (-1 and 1) are underrepresented (have 50% fraction relative to the other gray level values).
% The correct line should be like:
%     ImageTextures{iImage} = 2*round(ncs*rand(nr,nc,1)-0.5)/(ncs-1)-1;
%     % no edge problem, as rand() generates numbers on the OPEN interval
%     % (0,1).
