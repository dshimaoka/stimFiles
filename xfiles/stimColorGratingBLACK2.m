function SS = stimColorGratingBLACK2(myScreenInfo,Pars)
...
x = XFile('stimColorGratingBLACK2',pp);
...
WindowImage = ones(size(xx)); %double( sqrt(xx.^2+yy.^2)<=diam/2 );
...
for iImage = 1:SS.nImages
    ...
    ImageTextures{iImage} = -ones(nx,ny,3);
    for igun = 1:3
