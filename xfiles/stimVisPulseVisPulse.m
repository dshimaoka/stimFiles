...
pp{4}  = {'tonprim',     'Onset time of primer (ms)',               400,  1,    6000};
pp{5}  = {'toffprim',    'Offset time of primer (ms)',              900,  1,    6000};
pp{6}  = {'tontest',     'Onset time of test (ms)',                 1000,   0,    6000};
pp{7}  = {'tofftest',    'Offset time of test (ms)',                3000,   0,    6000};
...
pp{14}  = {'ctest',       'Contrast of test',                       10,    0,    100};
pp{15}  = {'oritest',     'Orientation of test (deg)',              90,    0,    360};
...
% pp_test{2}  = {'tftest',   'Temporal frequency (Hz*10)',   40,   1,   200};
% pp_test{3}  = {'sftest',   'Spatial frequency (cpd*100)',  5,    0,   1000};
% pp_test{4}  = {'ctest',    'Contrast (%)'                  50,   0,   100};
% pp_test{5}  = {'oritest',  'Orientation (deg)',            45,   0,   360};
% pp_test{6}  = {'dimtest',  'Diameter (deg*10)'             250,  1,   500};
% pp_test{7}  = {'dimtest',  'Diameter (deg*10)'             250,  1,   500};
% pp_test{8}  = {'x1',    'Focus position, x (deg*10)',   0,   -900, 900};
% pp_test{9}  = {'y1',    'Focus position, y (deg*10)',   0,   -300, 300};
% pp_test{10} = {'tontest',   'onset time(ms)',               300,  1,   6000};
% pp_test{11} = {'tofftest',  'offset time(ms)',              1300, 1,   6000};
% pp_test{12} = {'shape', 'shape (0 = circle; 1 = rect)', 1,    0,   1};
ParsTest = Pars([1 17 16 14 15 18 19 2 3 6 7 20]);
