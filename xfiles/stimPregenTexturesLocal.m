function SS = stimPregenTexturesLocal(myScreenInfo,Pars)
...
pp = cell(1,1);
pp{1}  = {'dur',    'Stimulus duration (s *10)',              100,1,6000};
...
if 1
    ...
    for iFrame = 1:ceil(SS.nFrames/(nfOFF + nfON))
        if rem(iFrame,2)==1
            iImage = ceil(iFrame/nFr1);
        else
            iImage = ceil(iFrame/nFr2);
        end
        SS.ImageSequence(iFrame) = iImage; % SS.ImagePointers(iImage);
    end
    ...
    blockGray(randperm(numel(blockGray), numel(blockGray)/2)) = NaN;
