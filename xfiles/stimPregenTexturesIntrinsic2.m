function SS = stimPregenTexturesIntrinsic2(myScreenInfo,Pars)
...
pp{6}  = {'nfON',    'number of frames stimulus stays on ', 60,1,200};
pp{7}  = {'nfOFF',   'number of frames stimulus stays off', 60,1,200};
pp{8}  = {'nfON0',      'number of frames per exemplar',  12, 1,200};
pp{9} = {'seed',   'Seed of random number generator',        1,1,100};

x = XFile('stimPregenTexturesIntrinsic2',pp);
...
for iFrame = 1:ceil(SS.nFrames/(nfOFF + nfON))    
    iImage = rem(iFrame-1, nClasses) + 1;
...
imgsq = repmat(SS.ImageSequence, round(2*nfON/nfON0),1);
...
imgsq = reshape(repmat(imgsq(:)', nfON0,1), nfON, []);
imgsq(randperm(numel(imgsq), numel(imgsq)/2)) = NaN;
