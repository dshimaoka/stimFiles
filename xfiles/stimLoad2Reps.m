function SS = stimLoad2Reps(myScreenInfo,Pars)
...
pp = cell(1,1);
pp{1}  = {'dur',    'Stimulus duration (s *10)',              5,1,18000};
pp{2}  = {'x1',     'Left border, from center (deg*10)',     -1400,-1400,1400};
pp{3}  = {'x2',     'Right border, from center (deg*10)',     1400,-1400,1400};
pp{4}  = {'y1',     'Bottom border, from center (deg*10)',   -600,-1000,1000};
pp{5}  = {'y2',     'Top border, from center (deg*10)',       600,-1000,1000};
...
pp{9}  = {'ip',     'which image database',                   101,1,1000};
pp{10}  = {'nimg',  'total number of images',                 500,1,10000};
pp{11}  = {'pbl',   'percentage blanks',                      5,1,100};
pp{12}  = {'shift', 'period of repeats',                      100,1,10000};

x = XFile('stimLoad2Reps',pp);
...
if repn>0
    iseries = rem(repn-1, 2*nshift)+1;
    if iseries > nshift
        iseries = iseries - nshift;
    end
    repn = floor((repn-1)/(2*nshift)) * nshift + iseries;
    
    iperm       = randperm(nimg + ceil(pblanks *nimg));    
    isample     = iperm(rem(repn-1, nimg + ceil(pblanks *nimg)) + 1);
else
