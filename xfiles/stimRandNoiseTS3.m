function SS = stimRandNoiseTS3(myScreenInfo,Pars)
...
x = XFile('stimRandNoise',pp);%% it needs to be stimRandNoiseTS3?? 20141126 
...
nFr = Pars(6);
nFr=nFr+mod(ceil(tsnum/tsstimnum),5);% introduce jitter 20120918
disp(['nFr = ', num2str(nFr)])
...
%     ImageTextures{iImage} = 2*round((ncs-1)*rand(nr,nc,1))/(ncs-1)-1;
      ImageTextures{iImage} = 2*round(ncs*rand(nr,nc,1)-0.5)/(ncs-1)-1; % 20140328 MK corrected line
