function SS = stim2Pulses2DurRandNoise_AL(myScreenInfo,Pars)
% stim2Pulses2DurRandNoise:
% make 1 or 2 pulses (can be of different wavelengths & lengths) + white noise
%
% SS = stim2Pulses2DurRandNoise(myScreenInfo,Pars) returns object SS of 
% type ScreenStim
% SS = stim2Pulses2DurRandNoise(myScreenInfo) uses the default parameters
%
...
% 2015-11-11 AL to have random inter pulse interval
...
pp{5}  = {'intT',      'Pulse-to-pulse interval (ms)',          5,0,1000};
...
x = XFile('stim2Pulses2DurRandNoise_AL',pp);
...
SS.Type = 'stim2Pulses2DurRandNoise_AL';
SS.Parameters = Pars;
rng(Pars(4), 'twister'); % seeds the random number generator
intT =  6 * rand;
ss = []; ss = Tp; if intT~=0, ss(end+1) = ss(end)+durT1+intT; end %2015-09-20
...
SS = stim2Pulses2DurRandNoise_AL(myScreenInfo); %#ok<UNRCH>
